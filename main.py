...
def fb_field_detector(filename: str) -> None:
    ...
    lower = np.array([40, 75, 37], dtype=np.uint8)
    ...
    mask = cv.morphologyEx(mask, cv.MORPH_CLOSE, kernel, iterations=8) # del holes
    mask = cv.morphologyEx(mask, cv.MORPH_OPEN, kernel, iterations=3) # del noise points
    kernel_h = cv.getStructuringElement(cv.MORPH_RECT, (81, 9))
    mask = cv.morphologyEx(mask, cv.MORPH_CLOSE, kernel_h, iterations=10)
